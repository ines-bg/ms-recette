...
public class RecetteController {
    ...
    @GetMapping("/validees")
    @Operation(summary = "Récupérer les recettes validées",
            description = "Récupère toutes les recettes ayant le statut VALIDEE")
    @ApiResponse(responseCode = "200", description = "Recettes validées récupérées")
    public ResponseEntity<List<RecetteResponse>> getRecettesValidees() {
        log.info("GET /api/recettes/validees - Récupération des recettes validées");
        List<RecetteResponse> recettes = recetteService.getRecettesValidees();
        return ResponseEntity.ok(recettes);
    }

    @GetMapping("/rejetees")
    @Operation(summary = "Récupérer les recettes rejetées",
            description = "Récupère toutes les recettes ayant le statut REJETEE")
    @ApiResponse(responseCode = "200", description = "Recettes rejetées récupérées")
    public ResponseEntity<List<RecetteResponse>> getRecettesRejetees() {
        log.info("GET /api/recettes/rejetees - Récupération des recettes rejetées");
        List<RecetteResponse> recettes = recetteService.getRecettesRejetees();
        return ResponseEntity.ok(recettes);
    }
